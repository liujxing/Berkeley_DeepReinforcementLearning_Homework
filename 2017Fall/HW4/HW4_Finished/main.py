...
class NnValueFunction(object):
    ...
    def __init__(self, ob_dim, **vf_params):
        ...
        self.loss = tf.reduce_sum(self.sy_output_vf-self.sy_target_vf)**2 / tf.to_float(tf.shape(self.sy_output_vf))/2.0
...
if __name__ == "__main__":
    ...
    if mode==0:
        #general_params = dict(gamma=0.97, animate=True, min_timesteps_per_batch=2500, n_iter=300, initial_stepsize=1e-3, logdir=None, seed=0, desired_kl=2e-3, vf_type="linear", vf_params={})
        #general_params = dict(gamma=0.97, animate=True, min_timesteps_per_batch=2500, n_iter=300, initial_stepsize=1e-3, logdir=None, seed=0, desired_kl=2e-3, vf_type="nn", vf_params=dict(n_epochs=10, stepsize=1e-3))
